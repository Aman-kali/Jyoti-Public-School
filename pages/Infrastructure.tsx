  ...
  return (
    <div className="bg-sky-50">
      <div className="bg-gradient-to-r from-primaryDark to-primary py-20 text-center text-white shadow-lg">
        <h1 className="text-5xl font-extrabold mb-3 text-shadow">Infrastructure</h1>
        <p className="text-blue-100 text-xl">Modern Facilities for Modern Learning</p>
      </div>
      ...
      <div className="container mx-auto px-4 py-20 space-y-32">
        ...
        {/* Health & Hygiene */}
        <section className="bg-gradient-to-br from-white to-sky-100 p-12 rounded-3xl text-center shadow-[0_20px_40px_rgba(0,0,0,0.05)] border border-sky-200 transform hover:scale-[1.01] transition-transform">
...
const InfraSection: React.FC<{ title: string, image: string, children: React.ReactNode, direction: 'ltr' | 'rtl' }> = ({ title, image, children, direction }) => (
    <div className={`flex flex-col md:flex-row gap-16 items-center ${direction === 'rtl' ? 'md:flex-row-reverse' : ''}`}>
        <div className="md:w-1/2 relative group perspective">
            ...
            <img 
                src={image} 
                alt={title} 
                className="relative z-10 rounded-2xl shadow-2xl w-full h-72 md:h-96 object-cover transform transition-transform duration-500 group-hover:-translate-y-2 group-hover:scale-[1.01] bg-white border-8 border-white" 
            />
        </div>
